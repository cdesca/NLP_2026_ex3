...
seqs = [str(r.seq) for r in records if len(str(r.seq)) % 3 == 0] # Keep only sequences that are multiples of 3 (codons)
print(len(seqs))

# Function to convert sequences into codons (triplets of nucleotides)
def to_codon(seq):
    return [seq[i:i+3] for i in range(0, len(seq), 3)]

codon_seq = [to_codon(seq) for seq in seqs] # list of a list
...
stoi = {s:i+1 for i,s in enumerate(codons)}
stoi['.'] = 0
itos = {i:s for s,i in stoi.items()}
print(itos)

# Building dataset of blocks of 31 codons
block_size = 31

def build_dataset(codon_seq):
  X, Y = [], []
  for nuc in codon_seq:
...
import random
random.seed(42)
random.shuffle(codon_seq)
n1 = int(0.8*len(codon_seq))
n2 = int(0.9*len(codon_seq)) # 80 10 10

Xtr, Ytr = build_dataset(codon_seq[:n1])
Xdev, Ydev = build_dataset(codon_seq[n1:n2])
Xte, Yte = build_dataset(codon_seq[n2:])

# Creating initial embedding matrix
C = torch.randn((len(stoi), 32))

g = torch.Generator().manual_seed(123456) # for reproducibility

# Initialising model parameters
C = torch.randn((65, 32), generator=g) # Character embedding matrix - 65 characters by 32 embedding size
W1 = torch.randn((block_size * 32, 120), generator=g) # Concatenated embeddings (3*32) by 120 neurons - hidden layer weights
b1 = torch.randn(120, generator=g) # 120 neurons - hidden layer biases
W2 = torch.randn((120, 65), generator=g) # 120 neurons, 65 output layer size - hidden layer weights
b2 = torch.randn(65, generator=g) # 65 output layer size - hidden layer biases
...
for i in range(30000): # Iterations
    ...
    if i % 1000 == 0:
        with torch.no_grad():
            emb_dev = C[Xdev]
            h_dev = torch.tanh(emb_dev.view(-1, block_size*32) @ W1 + b1)
...
start = set(['ATG', 'GTG', 'TTG'])
stop = set(['TAA', 'TAG', 'TGA'])

# Assigning colors based on codon type
colors = []
